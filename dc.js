...
client.on("ready", async () => {
  let sesliKanalID = client.channels.cache.get("1022587835703898135");
  if (sesliKanalID)
    sesliKanalID.join()
...
client.on("message", async msg => {
  ...
  client.on("message", async mesaj => {
    const ever = ["1022587840552509600"]
    if (ever.some(word => mesaj.content.includes(word))) {
      if(mesaj.author.id == "1022587840552509600") return;
    ...
    host: '185.254.30.118',
        ...
        const embed = new Discord.MessageEmbed()
        .setColor("#000000")
        .setTitle("**PMG Deathmatch Sunucu İstatistikleri**")
        .setThumbnail(mesaj.guild.iconURL({dynamic: true, type: 'png'}))
        .addField('Sunucu IP: ', "```185.254.30.118```")
...
setInterval (function () {
    ...
    host: '185.254.30.118',
		...
		client.user.setActivity('IP: 185.254.30.118:22003',{type: "WATCHING"})
		}else{
